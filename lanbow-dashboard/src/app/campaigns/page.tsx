...
import { Plus, Search, Filter, ArrowUpDown, Pause, Play, ChevronRight } from "lucide-react";
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input placeholder="Search campaigns..." className="pl-9 pr-4 py-2 text-sm bg-muted rounded-lg border-none focus:outline-none focus:ring-2 focus:ring-primary/30 w-56" />
          </div>
              ...
              <tr key={c.id} className="hover:bg-muted/30 transition group">
                <td className="px-5 py-4">
                  <Link href={`/campaigns/${c.id}`} className="block">
                    ...
                    <p className="text-xs text-muted-foreground">{c.product} · {c.market}</p>
                  </Link>
                </td>
                ...
                <td className={`px-3 py-4 text-right text-sm font-semibold ${healthColors[c.healthTag as keyof typeof healthColors]}`}>
                  {c.roas}x
                </td>
                ...
                <td className="px-3 py-4">
                  <div className="flex items-center justify-center gap-1">
                    <button className="p-1.5 rounded-lg hover:bg-muted transition" title={c.status === "active" ? "Pause" : "Resume"}>
